...
What is gone is the tuning scaffolding, which had nothing to do with recognising a face:
summary_*.json and frames_*.jsonl with the counters behind them, the enrollment-blocker
diagnostics, the memory-error explanations, and 26 of the 41 command-line options. The
thresholds are now plain constants below -- edit them here instead of passing flags. Use
_16 when you need to measure why a threshold blocks, then copy the values into this file.
