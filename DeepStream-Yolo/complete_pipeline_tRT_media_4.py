...
Utilizare:
    python3 complete_pipeline_tRT_media_4.py sample_vid.mp4
    python3 complete_pipeline_tRT_media_4.py sample_vid.mp4        # a doua oara: recunoaste
    python3 complete_pipeline_tRT_media_4.py sample/sample_vid.mp4 --database baza.json
    python3 complete_pipeline_tRT_media_4.py sample_vid.mp4 --reset-db --overwrite
...
class TrackState:
    def __init__(self, first_frame=0):
        ...
        self.best_unknown = None    # dict: embedding, quality, blur, size, frame
    ...
    def offer_unknown(self, embedding, quality, blur, size, frame_number):
        """Retine cea mai buna proba "necunoscut" a track-ului, pentru inrolare."""
        if self.best_unknown is None or quality > self.best_unknown["quality"]:
            self.best_unknown = {"embedding": embedding, "quality": quality,
...
class FaceSample:
    ...
    __slots__ = ("track_id", "state", "obj_meta", "crop", "blur", "size",
                 ...
                 "wants_sample", "is_candidate", "action")
...
def enroll_blockers(streak, score, size, blur, quality):
    """Ce anume a oprit inrolarea de data asta. Lista goala = s-a inrolat.
...
def process_frame(batch_meta, frame_meta, frame_number, frame_image,
    ...
    # Cand se deseneaza, landmark-urile trebuie pe toate fetele, ca sa se vada in
    # video. Cand nu (--no-video), le cerem doar pentru fetele care chiar
    # candideaza: un track e verificat o data la ~15 cadre si da 2-3 probe in
    # fereastra, deci raman de cateva ori mai putine treceri prin model.
    analysed = faces if DRAW_OVERLAY else [face for face in faces if face.wants_sample]
    ...
    # --- Pasul 4: care fete intra in concursul pentru "cel mai bun cadru" ---
    for face in faces:
        state = face.state
    ...
    # --- Pasul 5: pentru cine rulam recunoasterea in acest cadru ---
    to_recognize = []
    ...
    # --- Pasul 7: verificare si inrolare, secvential ---
    decisions = {}
    for (track_id, state), embedding in zip(to_recognize, embeddings):
        ...
        if AUTO_ENROLL and label == LABEL_UNKNOWN and not state.enrolled:
            ...
            # Prototipul se ia din cea mai buna proba de pana acum a track-ului,
            # nu din cea a ferestrei curente; pragurile de marime/blur/calitate se
            # judeca tot pe ea. Scorul ramane cel al verificarii curente: el spune
            # cat de sigur e ca persoana chiar nu e deja in baza.
            state.offer_unknown(embedding, quality, blur, size, frame_number)
            best = state.best_unknown
            blockers = enroll_blockers(state.unknown_streak, score,
                                       best["size"], best["blur"], best["quality"])
            if not blockers:
                label = enroll(best["embedding"])
                score = 1.0
                enrolled_now = True
                state.enrolled = True
                state.history.clear()
                RUN.enrollments.append({"nume": label, "cadru": frame_number,
                                        "track": track_id,
                                        "calitate": r(best["quality"]),
                                        "cadru_proba": best["frame"]})
            else:
        ...
        decisions[track_id] = {
            ...
            "blur": r(blur, 1), "marime": size, "streak": state.unknown_streak,
            "inrolat": enrolled_now,
        }
        if blockers:
            decisions[track_id]["blocaje"] = blockers
            # valorile pe care s-a judecat inrolarea (cea mai buna proba a
            # track-ului), ca sa se vada cat de departe e de praguri
            decisions[track_id]["cea_mai_buna_proba"] = {
                "calitate": r(best["quality"]), "blur": r(best["blur"], 1),
                "marime": best["size"], "cadru": best["frame"],
            }
    ...
    # --- Pasul 9: inregistrarea pentru log ---
    face_records = []
    for face in faces:
        state = face.state
        entry = {
        ...
        # Calitatea si punctele exista doar pentru fetele care au trecut prin
        # modelul de landmark-uri; puse pe zero pentru celelalte, ar strica si
        # cititul logului, si distributiile din summary.
...
def write_summary(run, args, output_dir):
    ...
    summary = {
        ...
        "sursa": {
            ...
            "praguri_scalate": getattr(args, "threshold_scaling", None),
...
def parse_args(argv=None):
    ...
    # Implicit None peste tot: asa se stie care praguri au fost cerute explicit
    # (alea raman neatinse) si care sunt cele din script (alea se scaleaza cu
    # rezolutia de lucru, in scale_size_thresholds).
    ...
    tuning.add_argument("--min-face", type=int, default=None,
                        help=f"latura minima a bbox-ului ca fata sa fie procesata "
                             f"(implicit {MIN_FACE_SIZE} la 1080p)")
    ...
    tuning.add_argument("--enroll-min-face", type=int, default=None,
                        help=f"implicit {ENROLL_MIN_FACE} la 1080p")
    ...
    tuning.add_argument("--no-auto-thresholds", action="store_true",
                        help="nu scala pragurile in pixeli cu rezolutia de lucru")
...
def apply_thresholds(args):
    ...
    global ENROLL_MIN_QUALITY, ENROLL_MAX_SCORE
...
THRESHOLD_REFERENCE_SIDE = 1080   # pragurile in pixeli sunt calibrate pe 1080p


def scale_size_thresholds(width, height, args):
    """Pragurile in pixeli, aduse la rezolutia de lucru.

    Acum ca rezolutia nu mai e fixa, "60 px" nu mai inseamna acelasi lucru de la
    un clip la altul: pe o filmare 720p aceeasi fata are cu o treime mai putini
    pixeli decat pe 1080p. Scalam dupa latura mica a cadrului (identica pentru
    1920x1080 si 1080x1920, deci orientarea nu schimba pragurile).

    Ce a fost cerut explicit in linia de comanda ramane cum a fost cerut.
    """
    global MIN_FACE_SIZE, ENROLL_MIN_FACE

    scale = min(width, height) / float(THRESHOLD_REFERENCE_SIDE)
    if args.no_auto_thresholds or abs(scale - 1.0) < 0.02:
        return None

    changed = {}
    if args.min_face is None:
        MIN_FACE_SIZE = max(24, int(round(MIN_FACE_SIZE * scale)))
        changed["MIN_FACE_SIZE"] = MIN_FACE_SIZE
    if args.enroll_min_face is None:
        ENROLL_MIN_FACE = max(32, int(round(ENROLL_MIN_FACE * scale)))
        changed["ENROLL_MIN_FACE"] = ENROLL_MIN_FACE
    return {"factor": round(scale, 3), "praguri": changed} if changed else None
...
def main():
    ...
    args.threshold_scaling = scale_size_thresholds(args.width, args.height, args)
    ...
    if args.threshold_scaling:
        print(f"        praguri in pixeli x{args.threshold_scaling['factor']}: "
              + ", ".join(f"{name}={value}" for name, value
                          in args.threshold_scaling["praguri"].items()))
