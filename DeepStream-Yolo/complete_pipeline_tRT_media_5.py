...
Utilizare:
    python3 complete_pipeline_tRT_media_5.py sample_vid.mp4
    python3 complete_pipeline_tRT_media_5.py sample_vid.mp4        # a doua oara: recunoaste
    python3 complete_pipeline_tRT_media_5.py sample/sample_vid.mp4 --database baza.json
    python3 complete_pipeline_tRT_media_5.py sample_vid.mp4 --reset-db --overwrite
...
class TrackState:
    ...
    def offer_unknown(self, embedding, quality, blur, size, frame_number):
        """Cea mai buna proba a track-ului DINTRE CELE care pot fi inrolate.
        ...
        """
        if sample_blockers(size, blur, quality):
            return
...
def sample_blockers(size, blur, quality):
    """Ce opreste o proba sa poata fi prototip. Lista goala = poate fi."""
    blockers = []
...
def enroll_blockers(state, score, size, blur, quality):
    ...
    if state.best_unknown is None:
        blockers.extend(sample_blockers(size, blur, quality) or ["fara_proba_buna"])
    return blockers
...
def process_frame(batch_meta, frame_meta, frame_number, frame_image,
    ...
    for (track_id, state), embedding in zip(to_recognize, embeddings):
        ...
        if AUTO_ENROLL and label == LABEL_UNKNOWN and not state.enrolled:
            ...
            blockers = enroll_blockers(state, score, size, blur, quality)
            if not blockers:
        ...
        if blockers:
            ...
            # proba buna si asteapta doar streak-ul, sau inca n-a prins niciuna
