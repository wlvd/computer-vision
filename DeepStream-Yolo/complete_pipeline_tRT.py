...
class TrtModel:
    ...
    def __init__(self, engine_path):
        ...
        try:
            ...
            # Daca engine-ul are batch dinamic, dimensiunea apare ca -1 si trebuie
            # concretizata acum, altfel formele iesirilor nu se pot calcula.
    ...
    def infer(self, array):
        """Ruleaza engine-ul si intoarce lista de iesiri, ca numpy.
...
_landmark_points = int(np.prod(landmark_model.outputs[0]["shape"])) // 2
...
def run_landmark(crop_bgr):
    ...
    landmark = landmark_model.infer(tensor)[0].reshape(-1, 2)
...
def get_embedding(aligned_bgr):
    ...
    emb = recognition_model.infer(img_chw)[0][0]
