...
Utilizare:
    ...
    python3 complete_pipeline_tRT_media.py sample_vid.mp4        # a doua oara: recunoaste
    python3 complete_pipeline_tRT_media.py sample/sample_vid.mp4 --database baza.json
    python3 complete_pipeline_tRT_media.py sample_vid.mp4 --reset-db --overwrite
...
def parse_args(argv=None):
    ...
    tuning.add_argument("--min-face", type=int, default=MIN_FACE_SIZE,
                        help="latura minima a bbox-ului ca fata sa fie procesata")
    tuning.add_argument("--min-blur", type=float, default=MIN_BLUR,
                        help="varianta Laplacianului considerata 'clar'")
    tuning.add_argument("--quality-min", type=float, default=QUALITY_MIN,
                        help="calitatea minima ca sa merite rulat modelul de "
                             "recunoastere")
    tuning.add_argument("--verify-threshold", type=float, default=VERIFY_THRESHOLD,
                        help="scorul cosinus de la care o fata e recunoscuta")
    tuning.add_argument("--enroll-min-checks", type=int, default=ENROLL_MIN_CHECKS)
    tuning.add_argument("--enroll-min-face", type=int, default=ENROLL_MIN_FACE)
    tuning.add_argument("--enroll-min-blur", type=float, default=ENROLL_MIN_BLUR)
    tuning.add_argument("--enroll-min-quality", type=float, default=ENROLL_MIN_QUALITY)
    return parser.parse_args(argv)


def apply_thresholds(args):
    """Muta pragurile din linia de comanda peste constantele modulului."""
