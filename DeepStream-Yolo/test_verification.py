...
MIN_BLUR = 250.0          # sub acest prag, crop-ul e prea slab pentru inrolare/verificare
ENROLL_FRACTION = 0.6     # procent din pozele fiecarei persoane folosit la inrolare
VERIFY_THRESHOLD = 0.42   # prag de decizie, estimat din testele tale anterioare

# Maparea manuala: ce track_id-uri (posibil fragmentate de tracker) apartin aceleasi persoane
IDENTITY_MAP = {
    "persoana_A": ["5"],
    "persoana_B": ["6"],
    "persoana_C": ["7", "8"],  # fragmentat de tracker, unificat manual aici
}
...
for identity, track_ids in IDENTITY_MAP.items():
    ...
    print(f"{identity}: {len(samples)} poze utilizabile (blur >= {MIN_BLUR})")
...
print(f"\nBaza de date: {len(database)} identitati -> {DB_PATH}")
