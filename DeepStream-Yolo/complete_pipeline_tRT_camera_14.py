...
Same processing as complete_pipeline_tRT_media_13.py, with two differences:

  1. the source is a camera -- v4l2 (USB webcam) or CSI (nvarguscamerasrc) -- and
     the default output is a window on screen. It can also record in parallel
     with the display (--record), or run with no image at all (--no-display).
  2. the presence of each identity is tracked, and when somebody leaves and comes
     back within RETURN_WINDOW_S seconds the console prints:

         [RETURNED] person_5 came back after 6.3 s

Compared with _12: everything is in English (comments, console output, CLI help
and the keys in the JSON files), and the identity carry-over between broken
tracks (a new track inheriting the name of one that had just disappeared in the
same spot) is gone. A new track id starts from nothing and gets its name back
only from the database, at its first check.

Usage:
    python3 complete_pipeline_tRT_camera_14.py                       # /dev/video0
    python3 complete_pipeline_tRT_camera_14.py --device /dev/video1
    python3 complete_pipeline_tRT_camera_14.py --camera csi --sensor-id 0
    python3 complete_pipeline_tRT_camera_14.py --record             # also writes mp4
    python3 complete_pipeline_tRT_camera_14.py --no-display         # console only
    python3 complete_pipeline_tRT_camera_14.py --return-window 30

The output folder (camera_<device> by default) is REUSED: the database inside it
is loaded at startup, so people enrolled yesterday are recognised today. The
database is also saved periodically during the run (--save-every), not only on
exit: a live session can last hours, and a hard Ctrl+C or a power cut must not
lose everything that was enrolled.

Presence counts only identities CONFIRMED by a pass through the recognition model
(checks_confirmed >= 1), and it also counts the tracks that did not pass the
gates in the current frame -- a turned head is not a departure. Two thresholds of
its own: ABSENCE_GRACE_S (below it, a gap is a flicker, not a departure) and
RETURN_WINDOW_S (above it, a reappearance is printed as [REAPPEARED], not
[RETURNED]).
"""
...
# Samples below MIN_BLUR still compete (an absolute threshold depends too much on
# the source); only this safety floor is dropped outright.
...
# Cadence for tracks that already have a confirmed identity. Between checks no
# landmarks are computed for them, which is the main speed lever; the cost is
# that an id swapped by the tracker is caught up to this many frames later.
...
# --- identity continuity ---
# What we know about a person (TrackState.identity) is kept apart from what the
# last check produced (the vote in history): a conclusion versus a measurement.
...
class FaceDatabase:
    ...
    def save(self):
        ...
        Here save() can also be called mid-run, from the GLib loop, while add()
        runs on the GStreamer thread; add() does the vstack before the append, so
        we snapshot both fields and write the consistent prefix.
        """
...
class PresenceLog:
    ...
    Keyed by database NAME, not by tracker id: one person can go through several
    ids without ever leaving. Time is the wall clock, because the window is in
    seconds and the frame rate varies on a live run.
    """
    ...
    def update(self, names, now):
        ...
        for name in names:
            ...
            if left_at is None:
                ...
                continue
            gone = now - left_at
            events.append(("returned" if gone <= self.window_s else "reappeared",
                           name, gone))
...
# Presence log, built in main() once the thresholds are known. Global like RUN,
# for the same reason: the GStreamer probe takes no extra parameter.
PRESENCE = None
...
def process_frame(frame_meta, frame_number, frame_w, frame_h, surface):
    ...
    # Gathered after the frame's decisions, so a return is announced in the very
    # frame it was confirmed. Held tracks count as present -- somebody turned away
    # has not left. Only identities confirmed by the recognition model
    # (checks_confirmed >= 1) trigger anything.
    ...
    for kind, name, gone in PRESENCE.update(present_names, time.monotonic()):
        if kind == "returned":
            ...
            print(f"[REAPPEARED] {name} is in frame again, after {gone:.1f} s "
                  f"(past the {RETURN_WINDOW_S:.0f} s window)")
    ...
    # Held boxes are counted even with no drawing, so the summary is the same
...
# A camera has no rotation tag, so rotation is requested explicitly with --rotate
# (useful for a sensor mounted sideways). nvvideoconvert flip-method: 1 = 90
...
def camera_kind(args):
    ...
    Nothing is probed beforehand: opening the camera to interrogate it and then
    closing it to reopen it from GStreamer is the shortest path to "Device or
    resource busy". The resolution is REQUESTED through caps instead.
    """
    if args.camera != "auto":
        return args.camera
...
def build_camera_source(args, kind):
    ...
    CSI delivers NVMM/NV12 directly. v4l2 delivers system memory (YUY2 or MJPEG),
    so it needs a CPU decode/convert before nvvideoconvert can upload it; --mjpeg
    is worth asking for when the camera supports it, since it puts far less
    ...
    # io-mode=2 (mmap) is what almost every UVC webcam wants; without it some
    # drivers fall back to read() and give much lower rates.
    ...
    if args.mjpeg:
        ...
        decoder = make_element(["nvjpegdec", "jpegdec"], "mjpeg-decoder")
    ...
    # videoconvert covers whatever format the camera emits; nvvideoconvert alone
    # does not accept them all and negotiation then fails with "not-negotiated".
    convert = make_element("videoconvert", "camera-convert")
...
def explain_camera_error(text, args):
    ...
    if "not-negotiated" in lowered or "internal data stream error" in lowered:
        ...
        print(f"         See what it can do:  v4l2-ctl -d {args.device} "
              f"--list-formats-ext")
...
def working_resolution(args):
    ...
    Note that the size thresholds are in pixels at the WORKING resolution, so
    asking for a smaller one shrinks the faces, not just the frame.
    """
    ...
    if args.width and args.height:
        ...
        if abs(given - actual) / actual > 0.01:
            print(f"[WARNING] --width/--height ({args.width}x{args.height}) have a "
                  f"different aspect ratio than the camera ({width}x{height}); the "
...
def build_pipeline(args, output_video):
    ...
    # Uploads to NVMM (v4l2) and scales to the working resolution.
    vidconv_in = make_element("nvvideoconvert", "convert-in")

    # Rotation happens before the capsfilter, so the caps already carry the
    # post-rotation sides (see working_resolution).
    if args.rotation:
    ...
    # live-source=1: the camera keeps producing whatever we do, so streammux must
    # not try to synchronise on timestamps.
    streammux.set_property('live-source', 1)
    # Tied to the camera rate so we do not add latency for nothing.
    ...
    # Output: window, recording, both (through a tee), or neither. All of them
    # share one nvdsosd -- the drawing is done once.
    branches = []

    def display_branch():
        convert = make_element("nvvideoconvert", "display-convert")
        # nv3dsink is the JetPack 5 sink and takes NVMM directly; nveglglessink
        # needs nvegltransform before it on Jetson.
        sink = make_element(["nv3dsink", "nveglglessink", "xvimagesink",
                             "autovideosink"], "display")
        factory = sink.get_factory().get_name()
        # sync=False: on a live source, show the latest processed frame instead of
        # waiting for its nominal timestamp.
    ...
    def record_branch():
        ...
        if encoder.get_factory().get_name() == "nvv4l2h264enc":
            ...
            # A keyframe every couple of seconds: the file stays readable from the
            # middle and a hard stop loses at most that much.
    ...
    # Each branch starts with its own queue, otherwise the slowest one (the
    # encoder) would hold up the window as well.
    if tee is not None:
        for index, branch in enumerate(branches):
            ...
            for upstream, downstream in zip([queue] + branch, branch):
                if not upstream.link(downstream):
                    raise RuntimeError(
                        f"Cannot link {upstream.get_name()} to "
                        f"{downstream.get_name()} (branch {index})."
                    )
...
def write_summary(run):
    ...
    summary = {
        ...
        # With queues the probe runs in parallel with the rest, so
        # probe_of_total can exceed 100%: it only means the probe is the long part.
        "speed": {
        ...
        # "history" holds every movement in order, so who was present at any
        # moment of the session can be reconstructed from it.
        "presence": {
            "movements": dict(PRESENCE.counts.most_common()) if PRESENCE else {},
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description="Live face recognition on a camera: a window on screen, "
                    "automatic enrollment, a persistent database and a console "
                    "announcement when somebody comes back into frame.",
    ...
    camera.add_argument("--sensor-id", type=int, default=None,
                        help="the CSI sensor index")
    camera.add_argument("--capture-width", type=int, default=1280,
                        help="the width REQUESTED from the camera; it must be one "
                             "the camera can deliver (v4l2-ctl --list-formats-ext)")
    ...
    camera.add_argument("--mjpeg", action="store_true",
                        help="ask for MJPEG instead of raw. Far less USB traffic at "
                             "the same resolution, so higher rates; costs a CPU "
                             "decode")
    parser.add_argument("--output", default=None,
                        help="the output folder (default: camera_<device>); if it "
    ...
    parser.add_argument("--width", type=int, default=None,
                        help="the WORKING width, if it must differ from the capture "
                             "one. Note that the size thresholds are in pixels at "
                             "this resolution, so shrinking shrinks the faces too")
    parser.add_argument("--height", type=int, default=None,
                        help="the working height; by default the capture one")
    parser.add_argument("--rotate", default="0", choices=["0", "90", "180", "270"],
                        help="rotation applied to the camera image, in degrees, "
                             "clockwise (for sideways-mounted cameras)")
    parser.add_argument("--bitrate", type=int, default=0,
                        help="the recording bitrate, in bits/s; "
                             "0 = computed from the resolution (~8 Mbit/s at 1080p)")
    parser.add_argument("--record", nargs="?", const=True, default=False,
                        help="also record an mp4, in parallel with the window; "
                             "without a value, into the run folder")
    parser.add_argument("--no-display", action="store_true",
                        help="do not open the window. Without --record only the "
                             "console is left: recognition, the announcements and "
                             "the database are unchanged, but the encoder and the "
                             "display sink leave the board")
    parser.add_argument("--no-frame-log", action="store_true",
                        help="do not write frames_*.jsonl. A one-hour live session "
                             "is ~100 000 lines that are usually never read; "
                             "summary.json is written anyway")
    parser.add_argument("--save-every", type=float, default=None,
                        help=f"how often the database is written to disk during the "
                             f"run, in seconds; 0 = only on exit "
                             f"(default {DATABASE_SAVE_INTERVAL_S})")
    parser.add_argument("--queue-size", type=int, default=4,
                        help="how many frames can wait in each queue; this decides "
                             "how much capture/detection, the probe and the output "
                             "overlap. Every waiting frame is an NVMM surface "
                             "(~8 MB at 1080p RGBA). 1 = practically no overlap")
    ...
    presence.add_argument("--return-window", type=float, default=None,
                          help=f"within how many seconds of leaving a re-entry is "
                               f"announced as a return; past it, it is an ordinary "
                               f"appearance (default {RETURN_WINDOW_S})")
    presence.add_argument("--absence-grace", type=float, default=None,
                          help=f"how many seconds without any box carrying their "
                               f"name mean the person really left (default "
                               f"{ABSENCE_GRACE_S})")
...
def explain_inference_error(text, args):
    ...
    print(f"  The camera delivers {source['width']}x{source['height']}, and on top "
          f"of that come the\n  detector, the two engines, the display sink and "
          f"(if asked for) the encoder.")
    print("  Things to try, in order:")
    print("  1. --no-display (without --record): removes both the display sink and "
          "NVENC.\n     Recognition, the announcements and the database are "
          "unchanged.")
    print("  2. --capture-width 1280 --capture-height 720: fewer pixels "
          "everywhere.\n     Careful: the faces shrink too, so the size thresholds "
          "are felt.")
...
def main():
    ...
    # Gst.init before any factory lookup: camera_kind and rotation_supported both
    # search for elements.
    Gst.init(None)
    args.camera_kind = camera_kind(args)
    if args.camera_kind == "v4l2" and not os.path.exists(args.device):
        print(f"[WARNING] {args.device} does not exist right now. Cameras the "
              f"system sees:  ls -l /dev/video*")
    ...
    if args.rotation and not rotation_supported():
        print(f"[WARNING] nvvideoconvert has no flip-method on this platform; I "
              f"cannot rotate the image by {args.rotation} degrees. The faces stay "
    ...
    if args.width < args.capture_width:
        ...
        print(f"         the image is shrunk {factor:.1f}x, and so are the faces: "
              f"the size thresholds are in pixels at the working resolution")

    # The folder is named after the camera, so its database is found again on the
    # next run and people enrolled yesterday are recognised today.
    ...
    print(f"Screen:  {'no (--no-display)' if args.no_display else 'window'}"
          + (f", recording into {output_video}" if args.record else ""))
    print(f"Presence: returns within {RETURN_WINDOW_S:.0f} s are announced "
          f"(a departure is declared after {ABSENCE_GRACE_S:.1f} s without any box "
          f"carrying the name)")
    ...
    # Periodic save: a live session can last hours, and a power cut or a second
    # Ctrl+C would lose everything enrolled in the meantime.
    ...
    # The second Ctrl+C stops immediately: if EOS never arrives (camera stuck in an
    # ioctl, pipeline stopped before PLAYING), the first one would look ignored.
    stopping = []
    ...
    def sigint_handler(sig, frame):
        ...
        print("\nStop requested; sending EOS so the file closes properly "
              "(another Ctrl+C stops immediately)...")
    ...
    try:
        ...
        if presence["movements"]:
            movements = ", ".join(f"{name} x{count}"
                                  for name, count in presence["movements"].items())
            print(f"  presence:          {movements}")
            for event in presence["returns"]:
                print(f"    {event['name']} came back after {event['gone_s']} s")
            if presence["in_frame_at_end"]:
                print(f"    in frame at stop: "
                      f"{', '.join(presence['in_frame_at_end'])}")
