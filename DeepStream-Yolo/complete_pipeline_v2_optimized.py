"""Varianta optimizata a lui complete_pipeline_v2.py. Aceeasi logica, alt cost.

CE S-A SCHIMBAT SI DE CE

1. Nu se mai copiaza cadrul intreg.
   v2 aducea ~8 MB din GPU si facea cvtColor RGBA->BGR pe 1920x1080, la FIECARE
   cadru, chiar si cand nu era nicio fata de verificat. Acum se decupeaza doar
   regiunea fetei direct din surface, si numai daca exista macar un candidat.

2. Inferenta a iesit din firul pipeline-ului.
   v2 rula landmark + aliniere + embedding chiar in probe, deci pipeline-ul statea
   cat dura. Costul crestea liniar cu numarul de fetelor si bloca afisarea. Acum
   probe-ul doar decupeaza si pune in coada; un fir separat face inferenta si scrie
   eticheta, care apare cu 2-3 cadre intarziere -- invizibil.

3. Verificarile nu se mai sincronizeaza.
   Cu un interval fix, fetele intrate in cadru impreuna se re-verificau toate pe
   acelasi cadru: varf de lucru intr-un cadru, apoi 14 goale. De aici senzatia de
   sacadat. Acum intervalul are un decalaj per track.

4. Track-urile deja identificate se verifica mai rar.
   Un track cu eticheta stabila si scor bun nu are nevoie de re-verificare la 15
   cadre; trece pe STABLE_VERIFY_INTERVAL_FRAMES.

5. Providerii ONNX se cer explicit, si se spune pe ce a ajuns sesiunea.
   Fara `providers=`, onnxruntime alege singur si poate cadea tacut pe CPU.
"""
...
# Rezolutia muxer-ului. Coordonatele din rect_params sunt in acest spatiu, deci
# tot aici se face si decuparea la margini.
MUXER_WIDTH = 1920
MUXER_HEIGHT = 1080
...
# Un track cu eticheta stabila si scor bun nu are nevoie de aceeasi frecventa.
STABLE_VERIFY_INTERVAL_FRAMES = 90
STABLE_MIN_SCORE = 0.55

# Coada catre firul de inferenta. Mica intentionat: daca s-a umplut, inseamna ca
# firul e in urma, si atunci e mai bine sa pierdem o verificare decat sa acumulam
# o coada de crop-uri vechi pe care le-am eticheta cu intarziere tot mai mare.
WORKER_QUEUE_SIZE = 8
...
def build_providers():
    """Providerii ceruti explicit, in ordinea in care onnxruntime ii incearca."""
...
def make_session(model_path):
    ...
    session = ort.InferenceSession(model_path, sess_options=options, providers=build_providers())
...
landmark_session = make_session(PFLD_MODEL_PATH)
...
recognition_session = make_session(RECOGNITION_MODEL_PATH)
...
if landmark_session.get_providers()[0] == "CPUExecutionProvider":
    print(
        "\nATENTIE: modelele ruleaza pe CPU. Pe JetPack 5.x nu exista\n"
        "onnxruntime-gpu — foloseste complete_pipeline_tRT.py, care merge pe\n"
        "TensorRT. Optimizarile de aici ajuta, dar nu inlocuiesc rularea pe GPU.\n"
    )
...
class TrackState:
    def __init__(self):
        ...
        self.pending = False       # are deja un crop in coada, nu mai punem altul

track_states = {}

# Un singur lock pentru starea track-urilor SI pentru baza de date. Ambele sunt
# scrise din firul de inferenta si citite din probe, iar inrolarea le atinge pe
# amandoua in aceeasi operatie.
...
work_queue = queue.Queue(maxsize=WORKER_QUEUE_SIZE)


def apply_result(track_id, crop_side, blur, result):
    """Scrie rezultatul unei verificari in starea track-ului. Ruleaza pe firul de inferenta."""
    with state_lock:
        state = track_states.get(track_id)
        if state is None:          # track-ul a disparut cat timp astepta in coada
            return
        ...
        if state.current_label != "necunoscut":
            print(f"[ALERTA] track {track_id} -> {state.current_label} (scor={score:.3f})")


def worker_loop():
    while True:
        item = work_queue.get()
        if item is None:           # semnal de oprire
            work_queue.task_done()
            break
        track_id, crop, blur, crop_side = item
        try:
            apply_result(track_id, crop_side, blur, process_face(crop))
        except Exception as error:
            print(f"Eroare la procesarea track-ului {track_id}: {error}")
            with state_lock:
                state = track_states.get(track_id)
...
worker_thread = threading.Thread(target=worker_loop, name="face-worker", daemon=True)
...
def full_pipeline_probe(pad, info, u_data):
    ...
    while l_frame is not None:
        ...
        # Sub lock: firul de inferenta scrie in track_states, iar iterarea peste
        # dictionar in timp ce alt fir il modifica arunca RuntimeError.
        ...
        # Primul pas: decidem cine are nevoie de verificare, FARA sa atingem
        # pixelii. Asa aflam daca merita sa mapam surface-ul.
        candidates = []
        ...
        while l_obj is not None:
            ...
            x2 = min(MUXER_WIDTH, int(rect.left + rect.width))
            y2 = min(MUXER_HEIGHT, int(rect.top + rect.height))
            w, h = x2 - x1, y2 - y1
            ...
            if confidence >= MIN_CONFIDENCE and w >= MIN_FACE_SIZE and h >= MIN_FACE_SIZE:
                with state_lock:
                    ...
                    if state.last_check_failed:
                        base_gap = RETRY_INTERVAL_ON_FAIL
                    elif (state.current_label
                          and state.current_label != "necunoscut"
                          and state.current_score >= STABLE_MIN_SCORE):
                        base_gap = STABLE_VERIFY_INTERVAL_FRAMES
                    else:
                        base_gap = VERIFY_INTERVAL_FRAMES

                    # Decalajul dupa track_id impiedica fetele intrate in cadru
                    # impreuna sa se re-verifice toate pe acelasi cadru.
                    gap = base_gap + (track_id % 7)

                    due = (not state.pending
                           and frame_number - state.last_checked_frame >= gap)
                    if due:
                ...
                if due:
                    candidates.append((track_id, x1, y1, x2, y2))

                # Eticheta se ia din valorile citite sub lock mai sus, nu direct
                # din state: firul de inferenta o poate schimba intre timp.
                display_text = f"{label} ({score:.2f})" if label else "verificare..."
        ...
        # Al doilea pas: abia acum atingem pixelii, si numai daca e cazul.
        # Surface-ul se mapeaza o singura data pe cadru, iar din el se copiaza
        # doar dreptunghiurile fetelor -- nu tot cadrul, ca in v2.
        if candidates:
            surface = pyds.get_nvds_buf_surface(hash(gst_buffer), frame_meta.batch_id)

            for track_id, x1, y1, x2, y2 in candidates:
                crop = cv2.cvtColor(
                    np.array(surface[y1:y2, x1:x2], copy=True, order='C'),
                    cv2.COLOR_RGBA2BGR,
                )
                ...
                try:
                    work_queue.put_nowait((track_id, crop, b_score, min(x2 - x1, y2 - y1)))
                except queue.Full:
                    # Firul de inferenta e in urma. Renuntam la verificarea asta;
                    # track-ul o va relua peste cateva cadre.
                    with state_lock:
                        state = track_states.get(track_id)
                        if state is not None:
                            state.pending = False
...
def main():
    ...
    try:
        ...
        # Oprim firul de inferenta si il asteptam, ca o inrolare in curs sa apuce
        # sa ajunga in baza de date inainte de salvarea finala.
        work_queue.put(None)
