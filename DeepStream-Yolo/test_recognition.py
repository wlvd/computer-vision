...
MODEL_PATH = "/workspace/_landmark/arcface.onnx"
...
def preprocess(img_bgr):
    ...
    return img_norm[np.newaxis, ...]  # NHWC: (1, 112, 112, 3)


def get_embedding(img_bgr):
    inp = preprocess(img_bgr)
    emb = session.run([output_name], {input_name: inp})[0][0]
    return emb / np.linalg.norm(emb)  # normalizare L2, standard pentru similaritate cosinus
...
for id_a, id_b in itertools.combinations(ids, 2):
    # comparam doar prima imagine din fiecare ID, ca sa nu explodeze numarul de perechi
